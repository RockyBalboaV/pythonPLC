...
def unpack(array_read, array_order, variables):
    ...
    for loop in range(len(array_order)):
        start_index = int(math.modf(variables[array_order[loop][1]].address)[1])
        i = 0
        while i < len(array_order[loop]):
            ...
            if type == 'BOOL':
                ...
                buffer.append([get_bool(array_read[loop], byte_index - start_index, bool_index), order])
            if type == 'BYTE':
                buffer.append([get_byte(array_read[loop], byte_index - start_index), order])
            if type == 'WORD':
                buffer.append([get_word(array_read[loop], byte_index - start_index), order])
            if type == 'DWORD':
                buffer.append([get_dword(array_read[loop], byte_index - start_index), order])
            if type == 'INT':
                buffer.append([get_int(array_read[loop], byte_index - start_index), order])
            if type == 'DINT':
                buffer.append([get_dint(array_read[loop], byte_index - start_index), order])
            if type == 'FLOAT':
                buffer.append([get_real(array_read[loop], byte_index - start_index), order])
            i = i + 2

    order0(buffer)  # 按最后一列顺序排序
    return buffer
...
def PDUorder(string, variables):
    ...
    if n > 0:
        ...
        for i in range(n - 1):
            ...
            if string[i + 1][1] - string[i][1] + offnum < 18 and len(s[j]) < 222:
                s[j].append([string[i + 1][0], string[i + 1][1]])
            else:
